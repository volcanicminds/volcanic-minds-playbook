...
AI transformation for SMEs is handled by our dedicated division, [Volcanic Minds AI](https://volcanicminds.ai){:target="_blank"}: readiness assessment, multi-agent architectures, EU AI Act governance.

We bring over a decade of collective experience across banking, manufacturing, healthcare, advertising and citizen services.
...
We keep people on a project rather than rotating them for internal convenience: continuity is worth more than resource optimisation. When someone does have to change, you hear it from us first, with a proper handover. You should never find out by noticing a different name in a commit.
...
Every project runs through four phases:

- **Engage**: we define vision, objectives, scope, timeline and resources. This is the plan everything else gets measured against.
...
The ceremonies matter less than what they are for: a partner should always be able to look at working software, not at a status report.
...
The code we develop for a partner is **100% theirs**. Legal transfer completes on final payment, which is standard practice in our industry, but ownership is the starting assumption of the contract, not a concession granted at the end of it.
...
- **We use AI tools daily**: code assistants, agents, model-based analysis. They make us faster. We don't hide it, and we don't bill hours that a tool saved.
...
- **Human-in-the-loop where it counts.** In the systems we build, AI prepares, analyses and proposes, but execution on critical processes requires human approval. We hold ourselves to the same rule.
...
Boundaries protect a project better than any contract clause. Here are ours.
...
A few rules keep meetings useful, for partners and colleagues alike:
...
We trust people to manage their own workload. We do not require collaborators to fill out timesheets unless the partner requests it or the contract stipulates it.

If you have a question or a concern about time tracking, raise it with your manager.
